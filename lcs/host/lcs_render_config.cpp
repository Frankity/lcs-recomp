...
// Sets `key` inside `section` of an ini file, keeping every other line as written.
bool write_ini_value(const std::filesystem::path &path, const std::string &section,
    ...
    const std::string wanted_section = lowercase_copy(section);
    const std::string wanted_key = lowercase_copy(key);
    const std::string entry = key + "=" + value;
    std::string current;
    std::size_t section_end = lines.size();
    bool section_found = false;
    bool replaced = false;
    for (std::size_t index = 0; index < lines.size() && !replaced; ++index) {
        const std::string line = trim_copy(lines[index]);
        if (line.size() >= 2u && line.front() == '[' && line.back() == ']') {
            if (section_found) {
                section_end = index;
                break;
            }
            current = lowercase_copy(trim_copy(line.substr(1u, line.size() - 2u)));
            section_found = current == wanted_section;
            continue;
        }
        if (!section_found || line.empty() || line[0] == ';' || line[0] == '#') continue;
        const std::size_t separator = line.find('=');
        if (separator != std::string::npos &&
            lowercase_copy(trim_copy(line.substr(0u, separator))) == wanted_key) {
            lines[index] = entry;
            replaced = true;
        }
    }
    if (!replaced) {
        if (section_found) {
            while (section_end > 0u && trim_copy(lines[section_end - 1u]).empty()) --section_end;
            lines.insert(lines.begin() + static_cast<std::ptrdiff_t>(section_end), entry);
        } else {
            if (!lines.empty() && !trim_copy(lines.back()).empty()) lines.emplace_back();
    ...
    for (const std::string &line : lines) output << line << (crlf ? "\r\n" : "\n");
...
bool lcs_save_config_value(const std::string &section, const std::string &key,
    ...
    {
        std::lock_guard<std::mutex> guard(global_configuration_mutex());
        path = global_configuration().source_path;
    }
    return !path.empty() && write_ini_value(path, section, key, value);
}
