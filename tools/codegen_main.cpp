...
void emit_target(std::ostringstream &body, std::uint32_t target,
                 ...
                 const std::set<std::uint32_t> *valid_units = nullptr) {
    ...
    if (unit_span_bytes != 0u && target >= executable_base) {
        ...
        if (valid_units == nullptr || valid_units->contains(unit)) {
...
struct GeneratedFunctionInput {
    ...
    const std::set<std::uint32_t> *valid_units{};
};
            ...
            if (decoded.has_delay_slot()) {
                ...
                if (is_branch(decoded.kind)) {
                    ...
                    if (likely_branch) {
                        ...
                        emit_target(body, target, function.entry_labels, "        ", function.executable_base, function.unit_span_bytes, function.direct_entry_ids, function.import_stubs, function.valid_units);
                        body << "    }\n";
                        emit_target(body, fallthrough, function.entry_labels, "    ", function.executable_base, function.unit_span_bytes, function.direct_entry_ids, function.import_stubs, function.valid_units);
                    } else {
                    ...
                    if (decoded.kind == psprecomp::OpcodeKind::J) {
                        ...
                        const bool target_in_span = function.unit_span_bytes != 0u &&
                            target >= function.executable_base;
                        const std::uint32_t target_unit = target_in_span
                            ? (target - function.executable_base) / function.unit_span_bytes : 0u;
                        const bool direct_unit = target_in_span &&
                            (function.valid_units == nullptr || function.valid_units->contains(target_unit));
                        if (!direct_unit)
                            body << "    ctx.pc = " << psprecomp::hex32(target) << "u;\n";
...
std::string lower_constant_gpr_writes(std::string text) {
    ...
    while ((search = text.find(needle, search)) != std::string::npos) {
        ...
        out.append(text, written, search - written);
        out += replacement;
        written = close + 1u;
        search = written;
    }
    out.append(text, written, text.size() - written);
    return out;
}
...
int generate_auto(const std::filesystem::path &elf_path,
    ...
    std::set<std::uint32_t> valid_unit_buckets;
    for (const auto &unit : units) valid_unit_buckets.insert(unit.bucket);
